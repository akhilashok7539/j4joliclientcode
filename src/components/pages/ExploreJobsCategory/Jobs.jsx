...
const Jobs = ({ district, url, job_no }) => {
  ...
  const getJobs = async () => {
    ...
    try {
      const res = await api.get('/job-seeker/explore-jobs', { params: param });
    ...
    <div className="jobs">
      <div className="card-container">
        {React.Children.toArray(
          jobs.map((job) => <JobCard job={job} url={url} job_category={CategoryName} />)
        )}
      </div>
      <Pagination pagination={pagination} currentPage={page} url={`${url}/${CategoryName}`} />
    </div>
  );
};
export default Jobs;
